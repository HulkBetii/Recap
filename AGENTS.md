...
- `series_recap.format` supports `compact` and `episode_chaptered`. `compact` keeps the older compressed season-summary behavior. Anime series defaults to `episode_chaptered`: beat 0 is a shared hook, then post-hook narration recaps each selected episode as ordered connected chapters.
- Anime series chaptered defaults use mode contribution ratios `full=0.22`, `quick=0.14`, `merge=0.05`, `skip=0.0` and `series_recap.tts_cps=24.0` calibrated for the current VBee voice speed plus composer underfill; a 24-minute quick episode should contribute roughly 3-4 minutes before TTS/runtime variation.
...
- `series_composer` builds an episode target plan inside `series_event_bank.json` with target duration, character budget, minimum characters, and beat budget per episode. ChatGPT still returns only selected `event_id` values; code derives source refs/timecodes and deterministic `series_chapters.json`.
- `series_composer` has deterministic QA plus one configurable Playwright revision pass (`series_recap.qa_max_revisions`, default `1`) for under-target length, repeated events, missing hook, missing episode chapters, per-episode under-budget chapters, or non-monotonic post-hook story/episode order. The final script remains JSON-only and still never accepts model-written timecodes.
...
- `config.anime.series.yaml` owns `series_recap` defaults: end-to-end scope, `format=episode_chaptered`, mode contribution ratios, Playwright composer runtime, and final matching clip limits.
