...
- `series_composer` has deterministic QA plus one configurable Playwright revision pass (`series_recap.qa_max_revisions`, default `1`) for under-target length, repeated events, missing hook, missing episode chapters, per-episode/arc under-budget narration, total season under-budget narration, non-story event selection, or non-monotonic post-hook story/episode order. Invalid revision JSON keeps the prior valid draft and records a warning in `series_composer.qa.json`.
