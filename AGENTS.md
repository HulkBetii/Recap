...
- Setuptools package discovery dùng allowlist cho các runtime package: `common`, `ingest`, `match`, `orchestrator`, `preflight`, `reaction_remix`, `render`, `review`, `shots`, `storymap`, `tts`, `visual_index`; `run.py` và `run_reaction.py` được đóng gói như py-module.
...
## 39. REACTION REMIX MVP IMPLEMENTATION

- `reaction-remix` là pipeline runtime song song với recap phim; không thay đổi ý nghĩa các contract `film_map.json`, `review_script.json`, `beats_timing.json`, `edl.json`, `run.py` hoặc renderer recap hiện có.
- Public entrypoint: `python run_reaction.py --input video.mp4 --run-dir runs/<name> --config config.reaction-remix.yaml`.
- Stage graph: `probe -> analyze/shots/stems -> segment -> plan -> write -> tts -> compose -> render -> qa`; ba stage `analyze`, `shots`, `stems` chạy song song sau probe.
- Package runtime nằm dưới `reaction_remix/`; orchestrator riêng hỗ trợ `--from`, `--to`, `--only`, `--force`, `--force-stage`, `--dry-run`, resume/cache và tối đa hai vòng repair.
- Contract version là `reaction-remix.v1`; mọi hash cross-stage là SHA-256 file thực dạng 64 ký tự hex. Models/validators dùng chung nằm trong `common/schema.py`.
- Artifact chính: `reaction_source.json`, `reaction_transcript.json`, `reaction_blocks.json`, `remix_plan.json`, `commentary_script.json`, `commentary_audio.json`, `commentary_fit_requests.json`, `audio_assets.json`, `remix_edl.json`, `render.timeline.json`, `render.command-manifest.json`, `reaction_remix.mp4`, `remix_qa.json`.
- R0 fail nếu thiếu primary video/audio hoặc có soft subtitle stream; giữ rational FPS, source identity và declaration burned-in subtitle.
- R1 dùng Faster Whisper `large-v3`, word timestamps, Lingua, SpeechBrain ECAPA + agglomerative cosine threshold `0.28`; primary regions tối đa `30s` ưu tiên cắt tại silence midpoint, sau đó full-timeline multilingual refinement bằng window `6s`/overlap `2s`. Short-window transcript được ưu tiên; primary transcript chỉ lấp đúng phần span chưa được refinement phủ, không được xóa cả primary turn vì overlap cục bộ. Region primary lỗi thành `analysis_gap`, không crash toàn file. Analyze algorithm hiện là v5.
- R2 dùng `segment.commentary_boundary_policy=strict_or_word_edge`. Mỗi cut point ghi additive `safety_mode`, `left_handle_s`, `right_handle_s`: `full_handle` yêu cầu đủ handle `120ms`; `word_edge` được phép thiếu handle nhưng không cắt xuyên word, không overlap content, chỉ có một narrator Nhật với language/speaker confidence `>=0.90`, và boundary confidence được khóa đúng `0.90`; `overlap` bị cap `0.89` và bắt buộc thành `mixed`. Cut không overlap/không xuyên word nhưng thiếu handle và không đủ điều kiện narrator được giữ như protected edge với `safety_mode=null`, confidence tối đa `0.89`; nó không được promote commentary nhưng vẫn giữ khả năng chia/reorder reaction. Commentary chỉ hợp lệ khi cả hai boundary là `full_handle|word_edge`; `source_boundary`, protected edge, `mixed`, `unknown` và analysis gap luôn được giữ. Block ID ổn định `block-0001`; event chỉ merge bảo thủ khi không thể đặt cut ngoài word/content overlap hoặc sẽ tạo block dưới `min_cut_spacing`. HTML audit `reaction_blocks.review.html` hiển thị safety mode/handle/downgrade; segment algorithm hiện là v7, và orchestrator chỉ reuse khi algorithm/input/config/output hashes trong meta đều còn khớp.
- R3/R4 chỉ dùng ChatGPT Playwright với profile cố định `D:\VibeCoding\auto_YT\data\chrome_user_data\PROFILE_GPT_1`; không có OpenAI text fallback. Reaction editorial default `reply_timeout_s=1200` vì prompt/QA dài có thể vượt 900s. LLM chỉ trả IDs/semantic/text, code sở hữu timecode và tự khôi phục reaction để giữ hard duration/retention. Recovery tìm prompt đã submit trong conversation (chuẩn hóa chrome UI như `Show more`) và reuse assistant response tương ứng trước khi cân nhắc gửi prompt mới.
- Unique reaction speech retention được tính từ union word timestamps/turn spans thật, không dùng toàn bộ block envelope. Preservation-first R3 giữ mọi block không phải commentary theo mặc định; ngoại lệ duy nhất là `plan.manual_drop_block_ids` do user/manual QA chỉ rõ vì còn giọng bình luận gốc hoặc đoạn quá khó xử lý sạch. Manual drop phải được ghi trong `excluded_blocks` với category `manual_drop`, chỉ áp dụng cho non-commentary blocks, và vẫn phải giữ unique reaction speech retention `>=0.90`. Commentary visual chỉ dùng đúng một block `kind=commentary` đủ dài và không được reuse bất kỳ source block nào đã có trên timeline.
- R5 khóa AI33 voice `elevenlabs_QPtBgsg1dxKTQHNpHrHt`, model label `eleven_multilingual_v2`, speed `1.0`, concurrency `1`, không fallback. `ja_basic` dùng NFC, bỏ control/zero-width và gom whitespace; cấm normalization tiếng Việt.
- TTS per-slot trim silence với handle `80ms`, target khoảng `-14 LUFS`, normalize có `0.3dB` codec headroom nhưng validation vẫn `<=-2 dBTP`, local Whisper JA similarity `>=0.90`; fit tolerance là `max(100ms, một frame)`, không time-stretch. Slot lỗi tạo `commentary_fit_requests.json`; direction `shorten|lengthen|clarify` chỉ rewrite/resynthesize slot bị ảnh hưởng.
- Demucs production tạo full-source `no_vocals` stem; dependency/separation failure là hard error. Chỉ slot bị đo thấy narrator leakage mới fallback sang TTS-only. Reaction placement bắt buộc source video/audio cùng span, gain `0dB`, speed `1.0`, không filter.
- Renderer mới chỉ nhận source CFR và giữ source resolution/FPS/sample rate/channel count, H.264 CRF18 + AAC192k; video-only concat + một PCM timeline rồi encode AAC một lần. Không scale/crop/fps conversion, subtitle/ASS/drawtext/overlay/delogo/blur/mask, freeze-frame hoặc tail padding.
- QA chỉ đo/ghi report: decode, frame/sample tiling, actual decoded frame/sample counts, AI33 provenance, unique reaction retention, loudness, click và silence. Mọi reaction/mixed/unknown placement được decode một lần rồi kiểm tra đầu/giữa/cuối; cửa sổ biên tối đa `0.5s` phải đạt correlation `>=0.98`, lag tối đa một frame, gain delta `<=0.3dB` và frame similarity `>=0.995`. Probe frame được clamp theo decoded source/output count, kể cả source-tail được reorder; visual probe ưu tiên đọc exact CFR frame index bằng OpenCV thay vì timestamp seek sát H.264/concat keyframe; failure luôn localize đúng `failed_placement_ids`. Hai cửa sổ cùng im lặng được coi là preserved với gain delta `0`, còn im lặng một phía vẫn fail. Leakage chỉ áp dụng cho commentary đã thay; report ghi `old_narrator_leakage_slot_ids` để repair đúng slot, còn `protected_narrator_overlap_block_ids` là warning/audit và không được sửa media protected. Command manifest có denylist visual filter.
- Default output ratio `0.875`; preferred `0.85–0.90` chỉ là advisory/audit. Acceptance thực tế cho video đã được duyệt là output ngắn hơn source và nằm trong hard `0.80–1.00`; unique reaction speech retention tối thiểu `0.90` vẫn là hard gate.
- Repair tối đa hai vòng: duration restore qua R3, TTS fit qua R4/R5, bed leakage qua R6 TTS-only, reaction media mismatch qua targeted R7 cache bypass. Reaction repair được remap theo `origin_block_id` sau mọi plan/compose change. Duration/bed repair chỉ trở thành durable sau khi repaired run pass toàn bộ QA; orchestrator lưu immutable accepted request dưới `work/orchestrator/accepted_repairs/` và ledger bind source/request/QA hash. Pending, legacy, corrupt hoặc mismatched repair artifact không được auto-apply.
- Optional extras: `reaction-analysis`, `reaction-audio`, và union `reaction-remix`. Version package vẫn giữ `1.0.2`; chưa tạo PR/release cho đến khi full authorized video vượt hard gate.
- Preservation-first POC R0-R8 `09:30–12:30` đã pass. R2 v7 phủ kín `184.201633s` bằng 39 block: `block-0003` và `block-0026` là replaceable commentary nhờ `word_edge`; narrator overlap cuối được bảo vệ nguyên trạng trong `block-0038` (`mixed`) và `block-0039` (`unknown`). Output dài `176.609767s` (`0.958785x`), giữ unique reaction speech `1.0`; 35 protected placements đạt min correlation `0.9986287014`, max lag `0ms`, max gain delta `0.0691078578dB`, min frame similarity `0.9950234368`. Hai AI33 slot dùng đúng voice/model, JA ASR similarity `1.0`, commentary leakage `0`; output peak chỉ tăng `0.1dB`, không click/silence/forbidden visual operation. Decode lệch `-1` frame và `-1318` sample nhưng nằm trong một-frame tolerance. Full R0-R2 v5/v7 đã rerun trên `1129.302494s`: 191 blocks phủ kín timeline, đủ 11 narrator anchors và cả 11 đều có replaceable core; phần overlap tại anchors 6, 8, 11 được bảo vệ trong `unknown/mixed`. Full R3-R8 hard QA đã pass trên `work/reaction-remix-full-r02`; sau manual QA, `block-0045` được drop bằng `plan.manual_drop_block_ids` vì còn giọng bình luận gốc/đoạn khó xử lý. Output hiện tại `1010.379002s` (`0.894693x`), ngắn hơn source `1129.302494s`, 160 protected placements, min reaction correlation `0.9971666`, max lag `0ms`, max gain delta `0.0931131dB`, min frame similarity `0.9954148`, 12 AI33 slots đúng provider/voice, min JA ASR match `0.9`, old narrator leakage `0`, forbidden visual operations `0`, click/silence `0`, protected overlap warning-only cho `block-0093`, `block-0127`, `block-0186`. User đã chấp nhận rằng target `960–990s`/preferred upper `0.90` không quan trọng; không cần R3 trim pass cho release nếu các hard gate khác vẫn pass.
