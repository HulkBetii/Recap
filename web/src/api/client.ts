import type {
  ...
  PlanResponse, PreflightResponse, Preset, QaMetric, RunQa, RunSummary, StageState,
} from "../types";
...
interface ApiJob { id: string; kind: "single" | "series"; status: Job["status"]; run_dir: string; attempt: number; heartbeat_at?: string | null; exit_code?: number | null; error_message?: string | null; created_at: string; started_at?: string | null; finished_at?: string | null; }
...
interface ApiRun { id: string; job_id?: string | null; kind: "single" | "series"; name: string; path_token: string; episode_keys: string[]; output_artifact_id?: string | null; execution_status?: Job["status"] | null; delivery_status: RunQa["status"]; modified_at: string; }
...
interface ApiPlan { plan_id: string; kind: "single" | "series"; command: string[]; run_dir: string; dag: { key: string; label: string; status?: string }[]; checks: ApiCheck[]; warnings: string[]; can_start: boolean; dry_run_output?: string; created_at?: string; }
...
function mapJob(job: ApiJob, stages: ApiStage[] = []): Job {
  ...
  return { id: job.id, kind: job.kind, title: fileName(job.run_dir), run_dir: job.run_dir, status: job.status, current_stage: running?.key ?? null, created_at: job.created_at, started_at: job.started_at, finished_at: job.finished_at, last_heartbeat: job.heartbeat_at, exit_code: job.exit_code, stages: mapped, error: job.error_message, queue_position: null };
}
function mapRun(run: ApiRun): RunSummary { return { id: run.id, job_id: run.job_id ?? undefined, kind: run.kind, title: run.name, run_dir: run.name, status: run.execution_status ?? "interrupted", delivery_status: run.delivery_status, episode_count: run.episode_keys.length || undefined, updated_at: run.modified_at, output_artifact_id: run.output_artifact_id }; }
...
export const api = {
  health: async (): Promise<HealthResponse> => {
    ...
    return { status: raw.status, active_job_id: raw.active_job?.id ?? null, queue_depth: 0, providers: raw.providers, checks: raw.runtime.map((check) => ({ key: check.code, label: check.code.replaceAll("_", " "), status: check.status, detail: check.message })) };
  },
  presets: async (): Promise<Preset[]> => (await request<{ id: string; name: string; token: string; kind: "single" | "series" }[]>("/presets")).map((item) => ({ ...item, path: item.name })),
  ...
  preflight: async (body: Record<string, unknown>): Promise<PreflightResponse> => { const raw = await request<{ can_start: boolean; checks: ApiCheck[]; warnings: string[] }>("/preflight", { method: "POST", body: JSON.stringify(body) }); return { can_start: raw.can_start, checks: raw.checks.map((check) => ({ key: check.code, label: check.code.replaceAll("_", " "), status: check.status, detail: check.message })), warnings: raw.warnings }; },
  plan: async (kind: "single" | "series", body: Record<string, unknown>): Promise<PlanResponse> => { const requestBody = { ...body }; delete requestBody.kind; const raw = await request<ApiPlan>(`/plans/${kind}`, { method: "POST", body: JSON.stringify(requestBody) }); return { plan_id: raw.plan_id, kind: raw.kind, command: raw.command, run_dir: raw.run_dir, stages: raw.dag.map(({ key, label }) => ({ key, label })), warnings: raw.warnings, can_start: raw.can_start, dry_run_output: raw.dry_run_output, expires_at: raw.created_at }; },
