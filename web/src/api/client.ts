...
interface ApiJob { id: string; kind: "single" | "series"; status: Job["status"]; run_name: string; title: string; attempt: number; heartbeat_at?: string | null; exit_code?: number | null; error_code?: string | null; error_message?: string | null; created_at: string; started_at?: string | null; finished_at?: string | null; }
...
interface ApiPlan { plan_id: string; kind: "single" | "series"; title: string; run_name: string; command_preview: string[]; dag: { key: string; label: string; status?: string; outputs?: string[] }[]; checks: ApiCheck[]; output_names?: string[]; warnings: string[]; can_start: boolean; dry_run_summary?: string; created_at?: string; }
...
function mapJob(job: ApiJob, stages: ApiStage[] = []): Job {
  ...
  return { id: job.id, kind: job.kind, title: job.title, run_name: job.run_name, status: job.status, current_stage: running?.key ?? null, created_at: job.created_at, started_at: job.started_at, finished_at: job.finished_at, last_heartbeat: job.heartbeat_at, exit_code: job.exit_code, stages: mapped, error: job.error_message, queue_position: null };
}
...
export const api = {
  health: async (): Promise<HealthResponse> => {
    const raw = await request<{ status: HealthResponse["status"]; runtime: ApiCheck[]; providers: Record<string, boolean>; active_job_id?: string | null }>("/health");
    return { status: raw.status, active_job_id: raw.active_job_id ?? null, queue_depth: 0, providers: raw.providers, checks: raw.runtime.map((check) => ({ key: check.code, label: check.code.replaceAll("_", " "), status: check.status, detail: check.message, details: check.details })) };
  },
  ...
  plan: async (kind: "single" | "series", body: Record<string, unknown>): Promise<PlanResponse> => { const requestBody = { ...body }; delete requestBody.kind; const raw = await request<ApiPlan>(`/plans/${kind}`, { method: "POST", body: JSON.stringify(requestBody) }); return { plan_id: raw.plan_id, kind: raw.kind, title: raw.title, run_name: raw.run_name, command_preview: raw.command_preview, stages: raw.dag.map(({ key, label, status, outputs }) => ({ key, label, status: status as StageState["status"], episode_key: key.includes(":") ? key.split(":", 1)[0] : undefined, outputs })), checks: raw.checks?.map((check) => ({ key: check.code, label: check.label ?? check.code.replaceAll("_", " "), status: check.status, detail: check.message, details: check.details })), output_names: raw.output_names, warnings: raw.warnings, can_start: raw.can_start, dry_run_summary: raw.dry_run_summary, expires_at: raw.created_at }; },
