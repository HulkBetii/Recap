...
def build_command(stage: str, paths: RunPaths, film: Path, config: dict[str, Any], force: bool, python_exe: str | None = None) -> list[str]:
    ...
    if stage == "ingest":
        ...
        for key in ("whisper_model", "gap_threshold", "max_vision_frames", "translate_model", "vision_model", "device", "asr_provider", "aligner", "transcript_input", "timecode_quality", "max_segment_s", "merge_gap_s", "openai_transcribe_model", "openai_chunk_s", "alignment_device", "transcript_correction", "glossary", "correction_model", "log_level"):
