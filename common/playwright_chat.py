...
async def _wait_streaming_done(
    page,
    timeout_s: int,
    ...
    while asyncio.get_event_loop().time() < deadline:
        try:
            assistant_locator = page.locator(ASSISTANT_MSG_SEL)
            ...
            if (
                ...
                and previous_assistant_text is not None
                and assistant_count > 0
            ):
                current_assistant_text = await _read_last_assistant_text_if_present(
                    assistant_locator,
                    assistant_count,
                )
                response_started = (
                    current_assistant_text is not None
                    and current_assistant_text != previous_assistant_text
                )
            stop_button_count = await page.locator(STOP_BUTTON_SEL).count()
            response_started = response_started or stop_button_count > 0
            if response_started and stop_button_count == 0:
                return
        except PlaywrightError as exc:
        ...
        if (
            ...
            logging.getLogger("common.playwright").info(
...
async def _read_last_assistant_text_if_present(locator, count: int) -> str | None:  # type: ignore[no-untyped-def]
    if count <= 0:
        return None
...
class PlaywrightChatClient:
    def __init__(
        ...
        *,
        ...
        self._context = None
        self._page = None
        self.logger = logging.getLogger("common.playwright")
    ...
    async def __aenter__(self) -> "PlaywrightChatClient":
        ...
        try:
            ...
            raise PlaywrightChatError(
                "ChatGPT prompt box was not found. Open the same profile manually and log in before running the pipeline.",
                code="login_required",
                retryable=False,
    ...
    async def ask(self, prompt: str) -> str:
        ...
        for attempt in range(1, self.max_attempts + 1):
            ...
            try:
                assistant_locator = self._page.locator(ASSISTANT_MSG_SEL)
                previous_assistant_count = await assistant_locator.count()
                try:
                    previous_assistant_text = await _read_last_assistant_text_if_present(
                        assistant_locator,
